        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            modelBuilder.Entity<Urun>()
                ...
                .UsingEntity<KategoriUrun>(
                    j => j.HasOne(x => x.Kategori).WithMany(x => x.KategoriUrunler).HasForeignKey(x => x.KategoriId)
                        .OnDelete(DeleteBehavior.Restrict), //  1----N
                    j => j.HasOne(x => x.Urun).WithMany(x => x.KategoriUrunler).HasForeignKey(x => x.UrunId)
                        .OnDelete(DeleteBehavior.Cascade), //  1----N
                    j =>
                    {
